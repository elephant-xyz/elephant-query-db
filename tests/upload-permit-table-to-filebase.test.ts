...
/**
 * Permit-table PUBLISH mechanics (safe, no-network).
 *
...
import {
  ...
  buildPermitTableGatewayUrls,
  buildPermitTableKey,
  defaultPermitTableIpnsLabel,
  geoIndexIpnsLabel,
  planPermitTableUpload,
  propertyIpnsLabel,
  queryTableIpnsLabel,
  resolvePermitTableIpnsLabel,
  uploadPermitTable,
} from "../scripts/upload-permit-table-to-filebase.js";
...
type UploadArgs = Parameters<typeof uploadPermitTable>[0];
...
function createMockS3Client(headerCid = "QmPermitTableHeaderCidXXXXXXXXXXXXXXXXXXXXXXX") {
...
describe("buildPermitTableKey / planPermitTableUpload — single object only", () => {
  it("derives a single permit-table key that is neither a properties/*, shards/*, nor the property query-table key", () => {
    const key = buildPermitTableKey("Lee");
    expect(key).toBe("query-tables/lee/permit-table.parquet");
    expect(key.startsWith("properties/")).toBe(false);
    expect(key.startsWith("shards/")).toBe(false);
    expect(key).not.toBe("query-tables/lee/query-table.parquet");
    expect(key).not.toBe("index.json");
    expect(key).not.toBe("manifest.json");
  });

  it("plans EXACTLY one object", () => {
    const plan = planPermitTableUpload({ county: "lee" });
    expect(plan.objects).toHaveLength(1);
    expect(plan.objects[0]?.key).toBe("query-tables/lee/permit-table.parquet");
  });
});
...
describe("resolvePermitTableIpnsLabel — separate-label contract", () => {
  it("defaults to oracle-permit-table-<county> when the override is unset", () => {
    expect(resolvePermitTableIpnsLabel(fullEnv(), "lee")).toBe(PERMIT_TABLE_LABEL);
    expect(defaultPermitTableIpnsLabel("lee")).toBe(PERMIT_TABLE_LABEL);
  });

  it("uses FILEBASE_PERMIT_TABLE_IPNS_LABEL when set", () => {
    const env = fullEnv({ FILEBASE_PERMIT_TABLE_IPNS_LABEL: "custom-label" });
    expect(resolvePermitTableIpnsLabel(env, "lee")).toBe("custom-label");
  });

  it("THROWS when the resolved label equals the property dataset label", () => {
    expect(propertyIpnsLabel("lee")).toBe(PROPERTY_LABEL);
    const env = fullEnv({ FILEBASE_PERMIT_TABLE_IPNS_LABEL: PROPERTY_LABEL });
    expect(() => resolvePermitTableIpnsLabel(env, "lee")).toThrow(/property dataset label/);
  });

  it("THROWS when the resolved label equals the geo-index label", () => {
    expect(geoIndexIpnsLabel("lee")).toBe(GEO_LABEL);
    const env = fullEnv({ FILEBASE_PERMIT_TABLE_IPNS_LABEL: GEO_LABEL });
    expect(() => resolvePermitTableIpnsLabel(env, "lee")).toThrow(/geo-index label/);
  });

  it("THROWS when the resolved label equals the property query-table label (clobber guard)", () => {
    expect(queryTableIpnsLabel("lee")).toBe(QUERY_TABLE_LABEL);
    const env = fullEnv({ FILEBASE_PERMIT_TABLE_IPNS_LABEL: QUERY_TABLE_LABEL });
    expect(() => resolvePermitTableIpnsLabel(env, "lee")).toThrow(/property query-table label/);
  });
});
...
describe("buildPermitTableGatewayUrls", () => {
  it("builds both the Filebase and dweb.link gateway forms from the network key", () => {
    const urls = buildPermitTableGatewayUrls(NETWORK_KEY);
...
describe("uploadPermitTable — single-object upload + IPNS recording", () => {
  ...
  it("PUTs exactly one object and re-points the permit-table label at the derived CID", async () => {
    const client = createMockS3Client();
    const { fetchImpl, calls } = createMockFetch();

    const result = await uploadPermitTable({
      client: asClient(client),
      fetchImpl: asFetch(fetchImpl),
      env: fullEnv(),
      county: "lee",
    ...
    expect(client.sent[0]?.key).toBe("query-tables/lee/permit-table.parquet");
    expect(result.key).toBe("query-tables/lee/permit-table.parquet");
    expect(result.ipnsLabel).toBe(PERMIT_TABLE_LABEL);
    ...
    expect(writes.map((c) => c.body?.["label"])).toContain(PERMIT_TABLE_LABEL);
    expect(writes.map((c) => c.body?.["cid"])).toContain(result.cid);
  });

  it("never writes the property, geo, or property query-table IPNS label, and refuses a query-table-label env", async () => {
    const client = createMockS3Client();
    const { fetchImpl, calls } = createMockFetch();
    ...
    await expect(
      uploadPermitTable({
        client: asClient(client),
        fetchImpl: asFetch(fetchImpl),
        env: fullEnv({ FILEBASE_PERMIT_TABLE_IPNS_LABEL: QUERY_TABLE_LABEL }),
        county: "lee",
        body: PARQUET_BODY,
      }),
    ).rejects.toThrow();
